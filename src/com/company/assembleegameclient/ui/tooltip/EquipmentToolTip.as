...
package com.company.assembleegameclient.ui.tooltip
{
    ...
    public class EquipmentToolTip extends ToolTip 
    {
        ...
        private function addSetActivateOnEquipTagsToEffectsList(_arg_1:int):void
        {
            ...
            var _local_8:XML;
            var _local_9:XML;
            var _local_2:XML = ObjectLibrary.getSetXMLFromType(_arg_1);
            var _local_3:int;
            for each (_local_4 in _local_2.Setpiece)
            {
                if (_local_4.toString() == "Equipment")
                {
                    if (((!(this.player == null)) && (this.player.equipment_[int(_local_4.@slot)] == int(_local_4.@itemtype))))
                    {
                        _local_3++;
                    };
                };
            };
            _local_5 = TooltipHelper.SET_COLOR_INACTIVE;
            _local_6 = TooltipHelper.NO_DIFF_COLOR;
            if (_local_2.hasOwnProperty("ActivateOnEquip2"))
            {
                _local_5 = ((_local_3 >= 2) ? TooltipHelper.SET_COLOR : TooltipHelper.SET_COLOR_INACTIVE);
                _local_6 = ((_local_3 >= 2) ? TooltipHelper.NO_DIFF_COLOR : TooltipHelper.NO_DIFF_COLOR_INACTIVE);
                this.setInfo.push(new Effect("2 Pieces", null).setColor(_local_5).setReplacementsColor(_local_5));
                for each (_local_7 in _local_2.ActivateOnEquip2)
                {
                    this.makeSetEffectLine(_local_7, _local_6);
                };
            };
            if (_local_2.hasOwnProperty("ActivateOnEquip3"))
            {
                _local_5 = ((_local_3 >= 3) ? TooltipHelper.SET_COLOR : TooltipHelper.SET_COLOR_INACTIVE);
                _local_6 = ((_local_3 >= 3) ? TooltipHelper.NO_DIFF_COLOR : TooltipHelper.NO_DIFF_COLOR_INACTIVE);
                this.setInfo.push(new Effect("3 Pieces", null).setColor(_local_5).setReplacementsColor(_local_5));
                for each (_local_8 in _local_2.ActivateOnEquip3)
                {
                    this.makeSetEffectLine(_local_8, _local_6);
                };
            };
            if (_local_2.hasOwnProperty("ActivateOnEquipAll"))
            {
                _local_5 = ((_local_3 >= 4) ? TooltipHelper.SET_COLOR : TooltipHelper.SET_COLOR_INACTIVE);
                _local_6 = ((_local_3 >= 4) ? TooltipHelper.NO_DIFF_COLOR : TooltipHelper.NO_DIFF_COLOR_INACTIVE);
                this.setInfo.push(new Effect("Full Set", null).setColor(_local_5).setReplacementsColor(_local_5));
                for each (_local_9 in _local_2.ActivateOnEquipAll)
                {
                    this.makeSetEffectLine(_local_9, _local_6);
                };
            };
        }
        ...
        private function makeSetEffectLine(_arg_1:XML, _arg_2:uint):void
        {
            ...
            {
                this.setInfo.push(new Effect(TextKey.INCREMENT_STAT, this.getComparedStatText(_arg_1)).setColor(_arg_2).setReplacementsColor(_arg_2));
            };
        }
        ...
        private function makeSupporterPointsText():void
        {
            ...
            for each (_local_1 in this.objectXML.Activate)
            {
                ...
                if (_local_2 == ActivationType.GRANT_SUPPORTER_POINTS)
                {
                    ...
                    this.supporterPointsText.setStringBuilder(new StaticStringBuilder().setString(("Campaign points: " + _local_1.@amount)));
        ...
        private function getSkull(_arg_1:XML, _arg_2:XML=null):void
        {
            ...
            var _local_6:int = this.GetIntArgument(_arg_1, "wisDamageBase", 0);
            var _local_7:int = this.GetIntArgument(_arg_1, "wisMin", 50);
            var _local_8:int = Math.max(0, (_local_3 - _local_7));
            var _local_9:int = int(((_local_6 / 10) * _local_8));
            var _local_10:Number = MathUtil.round((int((_local_8 / _local_4)) * _local_5), 2);
            var _local_11:ComPair = new ComPair(_arg_1, _arg_2, "totalDamage");
            _local_11.add(_local_9);
            var _local_12:ComPair = new ComPair(_arg_1, _arg_2, "radius");
            var _local_13:ComPair = new ComPair(_arg_1, _arg_2, "healRange", 5);
            _local_13.add(_local_10);
            var _local_14:ComPair = new ComPair(_arg_1, _arg_2, "heal");
            var _local_15:ComPair = new ComPair(_arg_1, _arg_2, "ignoreDef", 0);
            var _local_16:int = this.GetIntArgument(_arg_1, "hitsForSelfPuri", -1);
            var _local_17:int = this.GetIntArgument(_arg_1, "hitsForGroupPuri", -1);
            var _local_18:String = this.colorUntiered("Skull: ");
            _local_18 = (_local_18 + (("{damage}" + this.colorWisBonus(_local_9)) + " damage\n"));
            _local_18 = (_local_18 + "within {radius} squares\n");
            if (_local_14.a)
            {
                _local_18 = (_local_18 + "Steals {heal} HP");
            };
            if (((_local_14.a) && (_local_15.a)))
            {
                _local_18 = (_local_18 + " and ignores {ignoreDef} defense");
            }
            else
            {
                if (_local_15.a)
                {
                    _local_18 = (_local_18 + "Ignores {ignoreDef} defense");
                };
            };
            if (_local_14.a)
            {
            ...
            this.effects.push(new Effect(_local_18, {
                ...
                "hitsSelf":TooltipHelper.getPlural(_local_16, "Hit"),
                "hitsGroup":TooltipHelper.getPlural(_local_17, "Hit")
            }));
                        ...
                        if (!((((_local_6 == "") || (_local_6 == "false")) || (_local_6 == "0")) || (_local_3.@effect == "Stasis")))
                        {
                            switch (_local_5)
                            {
                                case ActivationType.HEAL_NOVA:
                                    ...
                                    _local_3.@damage = this.modifyWisModStat(_local_3.@damage, 0);
                                    break;
                                case ActivationType.COND_EFFECT_AURA:
