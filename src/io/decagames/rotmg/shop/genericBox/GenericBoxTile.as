...
package io.decagames.rotmg.shop.genericBox
{
    ...
    public class GenericBoxTile extends UIGridElement 
    {
        ...
        {
            var _local_1:ShopBoxTag = this.getTagByName(ShopBoxTag.LEFT_TAG);
            if (_local_1)
            {
        ...
        {
            var _local_1:int = ((this._boxInfo.unitsLeft == -1) ? int.MAX_VALUE : this._boxInfo.unitsLeft);
            var _local_2:int = ((this._boxInfo.purchaseLeft == -1) ? int.MAX_VALUE : this._boxInfo.purchaseLeft);
            return (Math.min(_local_1, _local_2));
        }

        private function clearTagContainer():void
        {
            var _local_3:ShopBoxTag;
